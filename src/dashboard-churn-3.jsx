 ...
 *
 * ✅ Mantém integralmente seu layout/UX original
 * ✅ Usa os mesmos MOCKS como fallback
 * ✅ Agora consome a API do backend para popular os gráficos
 *   Endpoints esperados (GET):
 *     /api/churn/trend
 *     /api/churn/waterfall
 *     /api/churn/summary_risco?dim=segmento|uf|fat_faixa
 *     /api/churn/queue
 *     /api/churn/nps_por_risco
 *     /api/churn/renovacao
 *     /api/churn/clusters_scatter (opcional)
 *
 * Dica: defina VITE_API_BASE com a URL do backend (ex.: http://localhost:5000)
 * ------------------------------------------------------------------ */
...
const API = import.meta.env?.VITE_API_BASE || ""; // vazio → mesmo host do front

// Conversões tolerantes (p/ strings tipo "1.234,56")
const toNum = (x) => {
  if (typeof x === "number") return x;
  if (typeof x !== "string") return safeNum(x);
  const s = x.replace(/\./g, "").replace(/,/g, ".").replace(/[^0-9+\-.]/g, "");
...
// ====== MOCKS originais (mantidos 100%) ======
const monthlyTrendMOCK = [
...
const npsPorRiscoMOCK = [
  { risco: "Baixo", nps: 8.3 },
  { risco: "Médio", nps: 6.9 },
  { risco: "Alto", nps: 5.4 },
];

const renovacaoJanelaMOCK = [
  { janela: "0–30", mrr: 620000, clientes: 33 },
  { janela: "31–60", mrr: 420000, clientes: 25 },
  { janela: "61–90", mrr: 310000, clientes: 19 },
];

// ====== Adaptadores (API → formato do dashboard) ======
const adaptTrend = (rows = []) => rows.map((r) => {
  const churn = toNum(r.churnRate ?? r.churn_logos_pct ?? r.churn_pct ?? 0);
  return {
    mes: r.mes || r.label || r.mes_label,
    churnRate: churn <= 1 ? churn * 100 : churn,
    revChurn: toNum(r.revChurn ?? r.revenue_churn ?? 0),
    grr: toNum(r.grr ?? r.grr_pct ?? 0),
    nrr: toNum(r.nrr ?? r.nrr_pct ?? 0),
  };
});

const adaptWaterfall = (rows = []) => rows
  .sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0))
  .map((r) => ({ etapa: r.etapa || r.label, valor: toNum(r.valor ?? r.mrr ?? 0) }));

const adaptSummary = (rows = [], dimLabel) => {
  const map = new Map();
  rows.forEach((r) => {
    const cat = r.categoria || r.cat || r[dimLabel] || r.fat_faixa || r.uf || r.segmento || "";
    const key = String(cat);
    const cur = map.get(key) || { cat: key, baixo: 0, medio: 0, alto: 0, mrr_baixo: 0, mrr_medio: 0, mrr_alto: 0 };
    const risco = (r.risco || "").toLowerCase();
    const n = toNum(r.clientes || r.qtd || 0);
    const m = toNum(r.mrr_total || r.mrr || 0);
    if (risco.startsWith("alto")) { cur.alto += n; cur.mrr_alto += m; }
    else if (risco.startsWith("méd") || risco.startsWith("med")) { cur.medio += n; cur.mrr_medio += m; }
    else { cur.baixo += n; cur.mrr_baixo += m; }
    map.set(key, cur);
  });
  return [...map.values()];
};

const adaptQueue = (rows = []) => rows.map((r, i) => ({
  id: r.id || i + 1,
  cliente: r.cliente || r.nome || r.cd_cliente || "-",
  mrr: toNum(r.mrr ?? r.mrr_total ?? r.valor ?? 0),
  risco: toNum(r.risco ?? r.risk_score ?? r.score ?? 0),
  cluster: r.cluster || r.cluster_label || r.grupo || "-",
  renovacao: toNum(r.renovacao ?? r.dias_renovacao ?? 90),
  uso30: r.uso30 ?? r.uso_30d ?? r.u30 ?? "-",
  tickets30: r.tickets30 ?? r.tickets_30d ?? r.t30 ?? null,
  sla: r.sla ?? r.sla_pct ?? null,
  nps: r.nps ?? r.nps_ult ?? null,
  motivos: Array.isArray(r.motivos) ? r.motivos : (r.motivos ? String(r.motivos).split(/;\s*/) : []),
  dono: r.dono || r.csm || "—",
  segmento: r.segmento || r.ds_segmento || "",
  uf: r.uf || "",
  faixa: r.faixa || r.fat_faixa || "",
}));

const adaptNps = (rows = []) => rows.map((r) => ({ risco: r.risco || r.level || "-", nps: toNum(r.nps ?? r.avg ?? 0) }));
const adaptRenov = (rows = []) => rows.map((r) => ({ janela: r.janela || r.bucket || "-", mrr: toNum(r.mrr ?? 0), clientes: toNum(r.clientes ?? r.qtd ?? 0) }));
...
export default function RiscoChurnWireframe() {
  ...
  // Dados (agora via API + fallback nos MOCKS originais)
  const [monthlyTrend, setMonthlyTrend] = useState(monthlyTrendMOCK);
  const [mrrWaterfall, setMrrWaterfall] = useState(mrrWaterfallMOCK);
  const [clientesData, setClientesData] = useState(clientesMOCK);
  const [distBy, setDistBy] = useState(distByMOCK);
  const [npsPorRisco, setNpsPorRisco] = useState(npsPorRiscoMOCK);
  const [renovacaoJanela, setRenovacaoJanela] = useState(renovacaoJanelaMOCK);
  ...
  const loadAll = async () => {
    const errs = {};
    try {
      const [t, w, sSeg, sUF, sFaixa, q, nps, ren, cls] = await Promise.all([
        getJSON(`${API}/api/churn/trend`).catch((e) => { errs.trend = e.message; return []; }),
        getJSON(`${API}/api/churn/waterfall`).catch((e) => { errs.waterfall = e.message; return []; }),
        getJSON(`${API}/api/churn/summary_risco?dim=segmento`).catch((e) => { errs.summary_seg = e.message; return []; }),
        getJSON(`${API}/api/churn/summary_risco?dim=uf`).catch((e) => { errs.summary_uf = e.message; return []; }),
        getJSON(`${API}/api/churn/summary_risco?dim=fat_faixa`).catch((e) => { errs.summary_faixa = e.message; return []; }),
        getJSON(`${API}/api/churn/queue`).catch((e) => { errs.queue = e.message; return []; }),
        getJSON(`${API}/api/churn/nps_por_risco`).catch((e) => { errs.nps = e.message; return []; }),
        getJSON(`${API}/api/churn/renovacao`).catch((e) => { errs.renov = e.message; return []; }),
        getJSON(`${API}/api/churn/clusters_scatter`).catch(() => []),
      ]);

      const trendA = adaptTrend(t);
      if (trendA.length) setMonthlyTrend(trendA);

      const wfA = adaptWaterfall(w);
      if (wfA.length) setMrrWaterfall(wfA);

      const dist = {
        Segmento: adaptSummary(sSeg, "segmento"),
        UF: adaptSummary(sUF, "uf"),
        "Faixa Faturamento": adaptSummary(sFaixa, "fat_faixa"),
      };
      if (Object.values(dist).some((v) => v.length)) setDistBy(dist);

      const qA = adaptQueue(q);
      if (qA.length) setClientesData(qA);

      const nA = adaptNps(nps);
      if (nA.length) setNpsPorRisco(nA);

      const rA = adaptRenov(ren);
      if (rA.length) setRenovacaoJanela(rA);

      // clusters_scatter é opcional — se vier vazio, o Scatter extra usa clientesData
    } catch (e) {
      errs.fatal = e.message;
    } finally {
      setErrors(errs);
    }
  };
  ...
  const distData = useMemo(() => {
    const base = (distBy && distBy[dim]) || [];
    return base.map((r) => {
      const total = safeNum(r.baixo) + safeNum(r.medio) + safeNum(r.alto);
      const baixo_pct = total ? (safeNum(r.baixo) / total) * 100 : 0;
      const medio_pct = total ? (safeNum(r.medio) / total) * 100 : 0;
      const alto_pct  = total ? (safeNum(r.alto)  / total) * 100 : 0;
      return { ...r, baixo_pct, medio_pct, alto_pct };
    });
  }, [distBy, dim]);

  // Fila priorizada (ordenada por impacto)
  const fila = useMemo(() => {
    return clientesData
      .filter((c) => c.risco >= riskMin)
      .filter((c) => (cat ? c.segmento === cat || c.uf === cat || c.faixa === cat : true))
      .sort((a, b) => b.risco * b.mrr - a.risco * a.mrr);
  }, [clientesData, riskMin, cat]);
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 py-5 space-y-6">
        ...
        <section className="grid grid-cols-2 md:grid-cols-6 gap-3">
          {[
            { label: "Churn de logos", value: monthlyTrend.length ? `${safeNum(monthlyTrend.at(-1)?.churnRate).toFixed(1)}%` : "—", tip: "Cancelamentos ÷ base inicial do período." },
            { label: "Gross Rev. Churn", value: fmtBRL(monthlyTrend.at(-1)?.revChurn || 0), tip: "Perdas (churn + contração)." },
            { label: "GRR", value: `${safeNum(monthlyTrend.at(-1)?.grr).toFixed(1)}%`, tip: "(MRR_fim – expansão – novos) ÷ MRR_início." },
            { label: "NRR", value: `${safeNum(monthlyTrend.at(-1)?.nrr).toFixed(1)}%`, tip: "(MRR_fim – novos) ÷ MRR_início." },
            { label: `Receita em Risco (${janela}d)`, value: fmtBRL(receitaEmRisco), tip: "MRR com score ≥ limiar e na janela selecionada." },
            { label: "Clientes em Risco / Save", value: `— / —`, tip: "Qtde em risco e Save Rate." },
          ].map((kpi, i) => (
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={monthlyTrend}>
                  ...
                  <XAxis dataKey="mes" />
                  <YAxis yAxisId={0} type="number" domain={[0, Math.ceil(Math.max(0, ...monthlyTrend.map(d=>safeNum(d.churnRate)))+1)]} />
            ...
            <div className="h-60">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={mrrWaterfall}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                  <XAxis dataKey="etapa" />
                  <YAxis type="number" domain={[Math.min(0, ...mrrWaterfall.map(d=>safeNum(d.valor)))-100000, Math.max(0, ...mrrWaterfall.map(d=>safeNum(d.valor)))+100000]} tickFormatter={(v)=>fmtBRL(v)} />
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart>
                  ...
                  <Scatter name="Contas" data={clientesData.filter(c=>c.risco>=riskMin)}>
                    {clientesData.filter(c=>c.risco>=riskMin).map((p, i)=> (
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart>
                ...
                <ZAxis type="number" dataKey="mrr" range={[60, 200]} />
                <Tooltip formatter={(v, n) => (n === "mrr" ? fmtBRL(v) : String(v))} />
                <Scatter name="Clientes" data={clientesData}>
        ...
        <section className="text-xs text-slate-500 px-1">
          ...
          {Object.keys(errors).length>0 && <p className="mt-1 text-red-500">Aviso: {JSON.stringify(errors)}</p>}
        </section>
      </main>
    </div>
  );
